...
class Required(BranchPattern):
    def match(self, left: List["Pattern"], collected: List["Pattern"] = None) -> Any:
        ...
        for pattern in self.children:
            ...
            if not matched:
                missing_required_items.append(pattern)
                final_rc = False
        return final_rc, left, collected
...
def docopt(
    ...
    if matched and left == []:
        ...
        return output_obj
    if left:
        dups = list(set([x.short for x in left
                         if isinstance(x, Option) and x.short is not None and x.argcount == 1]))
        dups.extend(list(set([x.longer for x in left
                              if isinstance(x, Option) and x.longer is not None and x.argcount == 1])))
        if len(dups) > 0:
            raise DocoptExit(f"Duplicate flags detected -- {dups}\n")
        unknowns = list(set([x.short for x in left
                             if isinstance(x, Option) and x.short is not None and x.argcount == 0]))
        unknowns.extend(list(set([x.longer for x in left
                                  if isinstance(x, Option) and x.longer is not None and x.argcount == 0])))
        if len(unknowns) > 0:
            raise DocoptExit(f"Unknown flags detected -- {unknowns}\n")

        raise DocoptExit(f"Error: found unmatched (duplicate?) arguments {left}")
    raise DocoptExit(f"Error: Missing required arguments/options")
